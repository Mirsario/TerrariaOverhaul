...
If you would like to make a sound play randomly, instead of acting like a loop, then:
- Remove the `IsLooped: true` bit from the `AmbienceTrack`'s `Sound` object.
- Add an instancing cooldown to the `AmbienceTrack` object:
  ```js
	AmbienceTrack: {
		// ...
  
		InstanceCooldown: {
			Min: 5.0 // The minimal amount of time in-between playbacks.
			Max: 80.0 // The maximum amount of time in-between playbacks.
			Exponent: 1.5 // Exponent value - the higher, the more likely it is that the rolled value will be closer to Max.
		}
	}
  ```
